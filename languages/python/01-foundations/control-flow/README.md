...
~~~bash
python scripts/automation.py run-module --module-path languages/python/01-foundations/control-flow
~~~
...
- In Python, read this module through the native focus ?Control Flow?; the shared folder name remains stable for side-by-side navigation.
- Use dynamic values, collection protocols, and context managers to demonstrate how to select branches that cover normal and boundary conditions.
...
```bash
python scripts/automation.py check-exercise --language python --level 01-foundations --module control-flow --exercise 01
```
