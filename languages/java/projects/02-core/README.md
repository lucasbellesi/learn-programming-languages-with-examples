...
- File input and output with `Path` and `Files`.
- Defensive parsing.
- Lists and records.
- Basic aggregation.
...
- Compared with C++, Java uses checked `IOException` to make file failure paths visible.
- Relative to Python, Java asks for more data-shape ceremony but gives clearer compile-time structure.
- The important comparison is explicit parsing versus concise file-processing syntax.
