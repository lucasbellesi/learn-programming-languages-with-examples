...
Model courses and enrollments with structs, methods, and guarded state updates.
...
```bash
python scripts/automation.py check-checkpoint --language go --kind project --level 03-advanced --solution
```

## Requirements

- Define a `Course` struct with title and capacity.
...
```bash
python scripts/automation.py check-checkpoint --language go --kind project --level 03-advanced
```
