...
~~~bash
python scripts/automation.py run-module --module-path languages/go/01-foundations/strings
~~~
...
- exercises/01.go: count words in an input line using robust whitespace handling.
- exercises/02.go: normalize input and detect whether it is a palindrome.
...
```bash
python scripts/automation.py check-exercise --language go --level 01-foundations --module strings --exercise 01
```
