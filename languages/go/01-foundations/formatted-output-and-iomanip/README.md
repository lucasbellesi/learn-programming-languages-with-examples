...
~~~bash
python scripts/automation.py run-module --module-path languages/go/01-foundations/formatted-output-and-iomanip
~~~
...
- exercises/01.go: collect product rows and print an aligned invoice table with totals.
- exercises/02.go: compute summary metrics and print them using user-selected precision.
...
```bash
python scripts/automation.py check-exercise --language go --level 01-foundations --module formatted-output-and-iomanip --exercise 01
```
