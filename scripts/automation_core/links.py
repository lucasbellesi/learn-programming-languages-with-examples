...
def resolve_candidate_path(root: Path, file_path: Path, destination: str) -> Path:
...
def check_file(root: Path, file_path: Path) -> list[str]:
    ...
    for link_type, raw_target in links:
        ...
        candidate = resolve_candidate_path(root, file_path, target)
...
def check_markdown_links(root: Path) -> list[str]:
    ...
    for file_path in markdown_files:
        broken_links.extend(check_file(root, file_path))
